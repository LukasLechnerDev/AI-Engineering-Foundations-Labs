...
scraped = scrape()
scraped.to_json(
    jobs_dir / "1-scraped_jobs.jsonl", orient="records", lines=True, force_ascii=False
)

filtered = filter_jobs(scraped)
filtered.to_json(
    jobs_dir / "2-filtered_jobs.jsonl", orient="records", lines=True, force_ascii=False
)
screened_count = len(filtered)

classified = classify(filtered, client)
classified.to_json(
    jobs_dir / "3-classified_jobs.jsonl",
    orient="records",
    lines=True,
    force_ascii=False,
)
...
summarized.to_json(
    jobs_dir / "4-summarized_jobs.jsonl",
    orient="records",
    lines=True,
    force_ascii=False,
)
...
with_skills.to_json(
    jobs_dir / "5-jobs_with_skills.jsonl",
    orient="records",
    lines=True,
    force_ascii=False,
)
