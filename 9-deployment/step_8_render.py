...
def render_email_html(df: pd.DataFrame, screened_count: int, project_dir: Path) -> str:
    print("\nStep 8b: Rendering email HTML...")

    html_template = Template(
        (project_dir / "email-template.html").read_text(encoding="utf-8")
    )
    raw_html = html_template.substitute(
        icon_img="",  # relative paths don't work in email clients
        verified_role_count=len(df),
        screened_job_count=screened_count,
        cards=build_cards(df),
    )
