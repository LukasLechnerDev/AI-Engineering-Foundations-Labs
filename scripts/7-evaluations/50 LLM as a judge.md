...
## Learning goals
- Understand why deterministic evals can't evaluate free-text / open-ended outputs like the classification reason
...
## Notes
- Anchor on the `reason` field from the classifier in `step_3_classify.py` — students already know this field exists
- The existing `test_reason_eval.py` is the implementation to walk through
...
But the classifier also returns a `reason` field — a short explanation of why a job was or wasn't classified as an AI engineering role.

How do you test if a reason is good?

You can't write `assert reason == "..."` — the text is different every run. A regex won't tell you whether the reason actually reflects the job description. You need something that understands language.
...
- The model's output (the classification + reason)
- A rubric that defines what "good" looks like

And it returns a structured verdict: pass or fail, and why.

The rubric is everything. A vague prompt like "is this a good reason?" produces vague, inconsistent judgments. A concrete rubric produces reliable signal. For our reason field, the rubric has three criteria:

1. **Grounded** — does it reference specific details from the job description, or is it making things up?
2. **Rule-aligned** — does it correctly apply the classification rules the model was given?
3. **Consistent** — does the reason logically support the classification decision?

The judge only passes if all three are met.

### Walking through test_reason_eval.py

Open `tests/test_reason_eval.py`. A few things to notice:

**The judge gets the classifier's own rules.** The `judge_instructions` string includes the original `instructions` variable from `step_3_classify.py`. The judge knows exactly what the classifier was supposed to do — so it can check whether the reason actually follows those rules.

**The judge uses a stronger model.** The classifier uses `gpt-4o-mini`. The judge uses `gpt-4o`. You want your judge to be smarter than — or at least as smart as — the model being judged.
...
| LLM-as-a-judge | Free-text quality (reason) | Judge LLM scores against rubric |
